...
const show = computed(() => {
  ...
  if (process.client) {
    const route = useRoute();
    return route.query._storyblok ? props.blok.preview : visitor.value;
  }
...
<template>
  <div v-if="show === 'new_visitor'">
    <StoryblokComponent v-for="currentBlok in blok.new_visitor_blocks" :key="currentBlok._uid" :blok="currentBlok" />
  </div>
  <div v-else>
    <StoryblokComponent
